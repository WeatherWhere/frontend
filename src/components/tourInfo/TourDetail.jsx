...
export default function TourDetail({
    ...
    if (modalInfo) {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/tour/detail?contentId=${modalInfo.contentId}&contentTypeId=${modalInfo.contentTypeId}`
        );
  ...
  return (
    <>
      {modalInfo &&
      (touristSpotState || cultureState || lodgingState || foodState) ? (
        <CommonWrap>
