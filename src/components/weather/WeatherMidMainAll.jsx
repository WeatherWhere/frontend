...
import {
  ...
  TD,
  Table,
} from "./weatherShortMainAll/WeatherShortMainAll";
...
export default function WeatherMidMainAll({ address }) {
  ...
  const getMidData = useCallback(async (key, token) => {
    await getWeatherMidForecast(key)
      .then((res) => {
  ...
  useEffect(() => {
    if (address) {
      ...
      getMidData(
        `${process.env.REACT_APP_BASE_URL}/weather/forecast/mid?region1=${address.region1}&region2=${region2}`
      );
    }
  }, [address, getMidData]);
            ...
            <tbody>
              ...
              <TR>
                ...
                {data.map((item, index) => (
                  <TD key={index}>
                    <StyledIcon
                      name={getMidStatus(item.wam)[0]}
                      color={getMidStatus(item.wam)[2]}
                      size="2rem"
                    />
                    <StyledIcon
                      name={getMidStatus(item.wpm)[0]}
                      color={getMidStatus(item.wpm)[2]}
                      size="2rem"
                    />
                  </TD>
                ))}
